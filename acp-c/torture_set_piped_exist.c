...
do_set_test(struct client *cli)
{
  ...
  uint32_t exist_count = maxcount * 0.99;
  ...
  if (!ok) {
    ...
    for (i=0; i<exist_count; i++) {
      rc = memcached_sop_insert(cli->next_mc, key, keylen, values[i], valuelengths[i], NULL);
      ok = (rc == MEMCACHED_SUCCESS);
      if (!ok) {
        print_log("sop create failed. id=%d key=%s rc=%d(%s)", cli->id, key,
          rc, memcached_strerror(NULL, rc));
  ...
  // Exist piped elements
  if (0 != client_before_request(cli))
    return -1;
    
  rc = memcached_sop_piped_exist(cli->next_mc, key, keylen,
  ...
  ok = (rc == MEMCACHED_SUCCESS && piped_rc == MEMCACHED_SOME_EXIST);
  do {
    if (!ok) break;
    for (i=0; i<exist_count; i++) {
      ok = (results[i] == MEMCACHED_EXIST);
      if (ok != 1) {
        print_log("sop piped exist failed. id=%d key=%s rc=%d(%s)",
                   cli->id, key, piped_rc, memcached_strerror(NULL, results[i]));
        break;
      }
    }
    if (!ok) break;
    for (; i<maxcount; i++) {
      ok = (results[i] == MEMCACHED_NOT_EXIST);
      if (ok != 1) {
        print_log("sop piped not_exist failed. id=%d key=%s rc=%d(%s)",
                   cli->id, key, piped_rc, memcached_strerror(NULL, results[i]));
        break;
      }
    }
...
torture_set_piped_exist_init(void)
{

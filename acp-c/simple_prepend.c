...
do_simple_test(struct client *cli)
{
  ...
  rc = memcached_add(cli->next_mc, key, keylen, (const char*)val_ptr,
    (size_t)val_len, 100 /* exptime */, 0 /* flags */);
  valueset_return_value(cli->vs, val_ptr);
  ok = (rc == MEMCACHED_SUCCESS);
  if (!ok) {
    print_log("add failed. id=%d key=%s rc=%d(%s)", cli->id, key,
  ...
  for (i = 0; i < 100; i++) {
    ...
    rc = memcached_prepend(cli->next_mc, key, keylen, (const char *)val_ptr,
	  (size_t)val_len, 600 /* exptime */, 0 /* flags */);
	ok = (rc == MEMCACHED_SUCCESS);
	if (!ok) {
      print_log("prepend failed. id=%d key=%s rc=%d(%s)", cli->id, key,
	    rc, memcached_strerror(NULL, rc));
	}
	if (0 != client_after_request(cli, ok)) {
      return -1;

...
do_list_test(struct client *cli)
{
  ...
  rc = memcached_lop_piped_insert(cli->next_mc, key, keylen,
                            MEMCACHED_COLL_MAX_PIPED_CMD_SIZE, indexes,
  ...
  if (!ok) {
    print_log("lop piped insert failed. id=%d key=%s rc=%d(%s)",
...
torture_list_piped_ins_init(void)
{
